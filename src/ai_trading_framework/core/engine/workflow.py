...
class WorkflowEngine:
    ...
    async def process(
        ...
        for recommendation in recommendations:
            ...
            await self._emit(run, EventType.RISK_EVALUATED, risk.model_dump(mode="json"))
            ...
            await self._emit(run, EventType.APPROVAL_REQUESTED, approval.model_dump(mode="json"))
            if simulate_approval and broker != BrokerName.PAPER:
                ...
                await self._emit(run, EventType.APPROVAL_GRANTED, approval.model_dump(mode="json"))
    ...
    async def preview_and_execute(
        ...
        order_request = OrderRequest(
            ...
            quantity=quantity,
            limit_price=recommendation.entry_price,
        )
        await self._emit(run, EventType.EXECUTION_REQUESTED, order_request.model_dump(mode="json"))
        ...
        await self._emit(run, event_type, result.model_dump(mode="json"))
        return preview, result

    async def _emit(self, run: RunRecord, event_type: EventType, payload: dict) -> None:
